...
public class SnowFlake {
    ...
    public SnowFlake(long datacenterId,long machineId){
        if(datacenterId > MAX_DATACENTER_NUM || datacenterId < 0){
            throw new IllegalArgumentException("datacenterId can not be greater than MAX_DATACENTER_NUM or less than 0");
        }
        if(machineId > MAX_MACHINE_NUM || machineId < 0){
            throw new IllegalArgumentException("machineId can not be greater than MAX_MACHINE_NUM or less than 0");
        }
    ...
    public synchronized long nextId(){
        ...
        lastStmp = currStmp;
        return (currStmp - START_STMP) << TIMESTMP_LEFT // 时间数部分
